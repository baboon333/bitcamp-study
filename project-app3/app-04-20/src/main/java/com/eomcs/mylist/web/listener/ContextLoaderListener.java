package com.eomcs.mylist.web.listener;       // 쓰다말음!!!!!!!!!!
...
public class ContextLoaderListener implements ServletContextListener {
  ...
  private void createBeans(String packageName) throws Exception {
    ...
    String packagePath = packageName.replace(".", "/"); // 파일 경로로 바꾸려면 /로 해야 한다.
  ...
  private void findComponent(File dir, String packageName) throws Exception {
    File[] files = dir.listFiles(file -> { // 익명함수 람다문법
      if (file.isDirectory()) return true; // 하위 디렉토리를 찾는다
      if (file.getName().endsWith(".class") && !file.getName().contains("$")) return true; // $ 붙은거는 중첩클래스이니
      return false;
    });

    for (File f : files) {
      if (f.isDirectory()) { // 디렉토리인 경우는 다시 찾는다. 디렉토리가 아니면 class 파일이니 출력을 해볼 것이다.
        findComponent(f, packageName + "." + f.getName());
      } else {
        // 패키지 이름을 포함한 클래스 이름을 알아낸다.
        String className = packageName + "." + f.getName().replace(".class", ""); // .class 확장자를 날리겠다. 그래야 원하는 패키지 이름을 포함한 클래스 class 이름이 생성된다.

        // 클래스 이름을 가지고 .class 파일을 찾아 로딩한다.
        Class<?> classInfo = Class.forName(className); // 패키지 이름을 포함한 클래스에서 클래스 이름만 뽑아보겠다.

        // 클래스 정보에서 @Component 애노테이션을 추출한다.
        Component compAnno = classInfo.getAnnotation(Component.class); // Component라는 애노테이션이 있냐? 있으면 꺼내줘
        if (compAnno == null) { 
        ...
        String rootPath = ""; // 클래스 위에 붙은 url
        RequestMapping requestMappingAnno = classInfo.getAnnotation(RequestMapping.class);
        if (requestMappingAnno != null) { // 있다면
          rootPath = requestMappingAnno.value(); // 그걸 rootPath로 삼겠다.
        }
