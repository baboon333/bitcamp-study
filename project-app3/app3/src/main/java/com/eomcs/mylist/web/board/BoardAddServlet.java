...
public class BoardAddServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    out.println("<form method='post'>"); // 같은 url일 경우는 action을 지정하지 않아도 된다.
    ...
    out.println("  location.href = 'list';"); // 리스트로 가라
    out.println("}");
    out.println("</script>");
    out.println("</body>");
    out.println("</html>");
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    board.setTitle(req.getParameter("title")); // 타이틀이란 이름으로 들어온 파라미터 값을 board에 담고
    ...
    resp.sendRedirect("list"); // 현재 url인 board에서 list로 가라!
  }
}
