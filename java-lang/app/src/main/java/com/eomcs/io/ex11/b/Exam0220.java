...
public class Exam0220 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // => 인스턴스 필드를 생성한 후 직접 필드에 값을 저장한다.(setter 메서드 대신)
    ...
    ObjectInputStream in = new ObjectInputStream(fileIn);  // FileInputStream에 ObjectInputStream 데코레이터를 붙인다.

    // Serialize로 출력된 데이터를 읽어 다시 원래의 객체로 만들기
    Member member = (Member) in.readObject(); // (Member)로 readObject()의 리턴 값은 member라고 알려줌
