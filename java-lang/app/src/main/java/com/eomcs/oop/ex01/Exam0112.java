...
public class Exam0112 {
  public static void main(String[] args) {
    ...
    //
    // [새_데이터_타입의_이름]
    // - 대문자로 시작한다.  (중요!!!!!!!!!! 이런거 모르면 무시당함.)
    // - 여러 단어가 결합된 이름인 경우 각 단어의 시작도 대문자로 작성한다.  (언더바 적지마. database에서는 이렇게 하지만 클래스 이름은 아님. 근데 같은 DBMS라도 각각의 규칙(관례)이 다름. 각 언어들도 코드 규칙이 있으니 그에 맞는 규칙에 따라야 함.)  // 변수명과 메소드명은 소문자로 쓴다.
    // - 보통 명사형으로 짓는다.(90%)  // 전치사구나 부사구, 형용사나 형용사구로 쓰기도 하지만 적다. 메소드의 경우는 90%가 동사구 형태이다.
    //

    // 다음은 성적 정보를 저장할 메모리 구조를 클래스로 설계한 것이다. (설계도)
    class Score {
      String name;
      int kor;
      int eng;
    ...
    Score s; // int 변수 선언하듯이 이렇게 하면 될까?       //score 주소를 담을 메모리를 만들어라!
    //
    ...
    //   => s 인스턴스의 필드 값을 설정한다.   // s가 인스턴스(객체)가 아니라 인스턴스를 가리키는 레퍼런스이다. s는 인스턴스의 주소를 저장하는 레퍼런스. 개발자들이 편하려고 s가 가리키는 인스턴스(객체)를 그냥 줄여말하는 것!
    //   => s 객체의 필드 값을 설정한다.
    //
    s.name = "홍길동"; // s에 저장된 주소로 찾아가서 name 항목에 값 저장
    s.kor = 100; // s에 저장된 주소로 찾아가서 kor 항목에 값 저장
    s.eng = 90;   //s가 가리키는 인스턴스의 eng 변수(필드)에 90을 집어넣어라
    s.math = 80;
...
//   변수 이름이 곧 메모리를 가리키는 이름이 된다. (주소가 아니라)
//     예) int age;
//
...
// - 문법:
//   레퍼런스명.항목명 = 값;
//   예) Score s = new Score();   // 여기서 Score s;만 주면 밑에서
//       s.name = "홍길동";       // null point exception이 발생한다. 찾아갈 주소가 없어~
//
