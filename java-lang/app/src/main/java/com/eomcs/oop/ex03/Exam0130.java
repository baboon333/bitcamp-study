...
public class Exam0130 {
  ...
  public static void main(String[] args) {
    ...
    // 클래스 변수 사용법
    // 클래스명.스태틱변수명 = 값;
    // 클래스를 사용하는 순간 클래스가 로딩되고, 스태틱 변수는 자동 생성된다. (그림 확인)
    A.v1 = 100;
    A.v2 = true;
...
// loading: 명령어나 데이터를 처리하기 위해 하드의 것을 메모리로 올리는 것
// ram의 데이터를 하드 디스크 쪽으로 보내는걸 save(저장)이라 한다. 
...
//      - 메서드는 생성하지 않는다! (메서드는 method area에!)
//    - 가비지 컬렉터는 이 메모리의 가비지들을 관리한다.
// 2) JVM Stack
//    - 각 스레드가 개인적으로 관리하는 메모리 영역이다.  (스레드 아직 안배움!)
