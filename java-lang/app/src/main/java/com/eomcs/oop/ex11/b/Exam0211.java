...
class B2 {
  ...
  static class X { // X는 B2의 스태틱 멤버이다!
    void test() {
      ...
      // 클래스 X도 B의 test() 메서드처럼 B2 클래스의 멤버이기 때문에 
      // B2 클래스의 멤버에 접근할 때는 
      ...
      //      v2 = 100; // 컴파일 오류! 스태틱 멤버는 인스턴스 주소를 받지 못하니까 인스턴스 멤버에 접근할 수 없다.
      //      m2(); // 컴파일 오류! 인스턴스 멤버는 무조건 앞에 인스턴스 주소를 넘겨줘야 하니! 인스턴스주소.m2();가 되어야 한다.
    }
  }
}
