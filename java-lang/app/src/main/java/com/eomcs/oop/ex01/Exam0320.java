...
public class Exam0320 {
  public static void main(String[] args) {
    ...
    s1 = new Score();  //인스턴스를 생성하면 각 필드에 기본값이 설정된다. 이것의 경우 (null, 0, 0, 0, 0, 0.0) -> 레퍼런스 변수는 null으로, 정수 변수는 0으로, 부동 소수점 변수는 0.0으로, 논리 변수는 false로  -> 어쨌든 메모리에서는 다 0으로 처리된다!

    // 새 인스턴스를 만들어 s1에 주소를 저장한다.
    s1 = new Score();

    // 그러면 기존에 들어 있던 주소는 잃어 버린다.   // 기존 인스턴스의 주소를 갖고 있는 레퍼런스가 단 한개도 없어져서 사용할 수 없다.(접근할 수 없다)
