...
// # 사용자 정의 데이터 타입 + 인스턴스 메서드(non-static)
//
public class Exam0130 {
  ...
  public static void main(String[] args) {  //public static끼리는 순서 바꿀 수 있다. static public -> 가능 
    Score s = new Score();
    s.name = "홍길동";
    s.kor = 100;
    s.eng = 90;
    ...
    s.calculate(); // 마치 변수 뒤에 연산자를 놓는 i++ 의 예와 비슷하다.  // 그래서 메서드를 연산자라고 하는 것이다.

    System.out.printf("%s, %d, %d, %d, %d, %.1f\n", s.name, s.kor, s.eng, s.math, s.sum, s.average);
  }

  // 클래스를 정의하는데 필드 선언을 사용해서 메모리 구조를 설계하고 메서드라는 문법을 사용해서 그 데이터를 다룰 연산자를 정의하는 것. 
  // 연산자를 만드는 방법은 static을 사용해서 만드는 방법(exam0120)과 non-static(exam0130)을 사용해서 만드는 방법이 있다.
}
...
//
// 2) 인스턴스 메서드(논스태틱 메서드)
// - 메서드 선언부에 static이 붙지 않는다.                     //static으로 정의한 메서드는 파라미터로 주소를 받아야하는데 논스태틱 메서드가 class이름 대신 다루고자 하는 값을 값에 주기 떄문에 간편하다! 
