...
public class Exam0340 {
  ...
  public static void main(String[] args) {
    ...
    //소수점을 뺀 후 7자리 숫자까지는 거의 정상적으로 저장된다.(유효자릿수가 6자리라고 하는 책도 있고 7자리라고 하는 책도 있다.)
    ...
    System.out.println(987654321.1234567f); //9.8765434E8 -> E8은 *10^8이란 소리임! 소수점 뒤가 잘렸음.
    System.out.println(9.876543211234567f);
    System.out.println(987654321123456.7f);
    System.out.println("----------------------------");

    //## 8byte(double) 부동소수점의 유효자릿수
    //소수점을 뺀 후 16자리 숫자까지는 거의 정상적으로 저장된다.(유효자릿수가 15자리라고 하는 책도 있고 16자리라고 하는 책도 있다.)
    System.out.println(987654321.1234567);  //그대로 출력된다.
    System.out.println(9.876543211234567);
    System.out.println(987654321123456.7);  //9.876543211234568E14 ->왜 마지막 숫자가 7이 아니라 8인가? 유효숫자라 하더라도 *2를 해서 소수점 계속 없어주는데 *2 해주는데 딱 안떨어지고 계속 0으로 안떨어지고 반복됨! 0.7같은거!!! 그래서 아무리 *2를 해서 소수점 이하를 날리려고 해도 날릴 수 없는 수가 존재한다. 따라서 JVM이 소수점 이하가 너무 길어지는걸 알아서 처리한 다음 대충 반올림해서 보여줌 . 987조에서 0.1은 아무것도 아니니 JVM이 알아서 대충 처리한거임. 완벽히 2진수로 안바뀌는 부동 소수점이 있다! 
    System.out.println("----------------------------");
    ...
    System.out.println(7 * 0.1); //결과: 0.7000000000000001 맨끝의 1이 부동소수점의 문제! 쓰레기가 붙음. 그렇기 때문에 7 * 0.1 == 0.7 처럼 부동소수점은 함부로 비교를 하는게 아니다! 저렇게 하면 FALSE가 나오기 때문
    //- 이유
