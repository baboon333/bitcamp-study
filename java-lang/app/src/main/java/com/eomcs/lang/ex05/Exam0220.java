...
public class Exam0220 {
  public static void main(String[] args) {
    ...
    // 결과는 false이다.
    // 이유?
    // - 부동소수점 값을 연산할 때 IEEE 754 명세에 따라   //2 곱해주는거! 딱 안떨어지는 애는 이렇게 하다가 쓰레기 값이 붙음
    ...
    // => JVM이 자동으로 처리하지 않는다.
    // => 다음과 같이 개발자가 직접 처리해야 한다.  double EPSILON = 0.00001;이렇게 부동소수점 값은 개발자가 정하기 나름. 우리가 기준 정하고 그거보다 작으면 둘은 같은거야~ 이렇게 보는거임.
    double EPSILON = 0.00001;  //01로 해도 되고 내가 정하기 나름~  //실무에서 이렇게 코드 짜면 된다!
    System.out.println(Math.abs((d1 + d2) - (x + y)) < EPSILON);  //Math.abs 절댓값!
  }
}
//부동소수점은 비교를 하지마! 그 둘을 뺀 다음에 그 차이가 극소수라면 그 둘은 같다고 보자! 이렇게 판단하자!