...
public class Exam0151 {
  ...
  static class Student {
    ...
    @Override  // 이클립스가 만들어준거잖아! 해석하려고 하지마~ 그냥 데이터가 같으면 같은 해시코드를 리턴한다고만 알아둬~
    public int hashCode() {
      final int prime = 31;
      int result = 1;
  ...
  public static void main(String[] args) {
    ...
    set.add(s1);
    set.add(s2);  // 이미 s2의 해시값과 같은 객체(s1)가 들어 있기 때문에 중복을 막기 위해 s2는 저장되지 않는다. //이미 s2와 같은 해시코드 값을 갖는 s1이 저장되어 있으니 같은 데이터 중복이 안되는 집합에는 s2가 저장이 안된다
    set.add(s3);
    set.add(s4);
