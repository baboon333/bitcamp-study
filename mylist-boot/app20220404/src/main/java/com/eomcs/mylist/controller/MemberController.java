...
public class MemberController {
  ...
  public Object facebookLogin(String accessToken, HttpSession session) {
    ...
    Map<String,String> result = restTemplate.getForObject( // get 요청을 하는 method
        "https://graph.facebook.com/v13.0/me?access_token={value1}&fields={value2}", // 요청할 URL 
        Map.class, // 서버에서 받은 결과의 타입 // 리턴값을 map 객체에 담아라(xml 형식 혹은 무슨 형식이어야 된대 1시간 7분쯤(3시 22분)에 말함)
        accessToken, // URL의 첫 번째 자리에 들어갈 값
        "id,name,email,gender" // 페이스북 측에 요청하는 로그인 사용자 정보
        );

    // 2) 사용자 이름과 이메일을 알아낸다.
    String name = result.get("name"); // Map 에서 페이스북이 보내준 이름과 email을 꺼내서 담는다.
    ...
    if (member != null) {
      ...
      return new ResultMap().setStatus(SUCCESS).setData("새 회원 로그인"); // 그 다음 로그인 처리를 해버린다.
    }
  }
}
