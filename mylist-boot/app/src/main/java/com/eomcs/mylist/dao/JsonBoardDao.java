...
public class JsonBoardDao implements BoardDao {
  ...
  public JsonBoardDao() {
    try {
      ...
      boardList.addAll(mapper.readValue(new File(filename), Board[].class)); // 파일을 읽고 Board[] 배열로 리턴해줘
  ...
  private void save() throws Exception {
    ObjectMapper mapper = new ObjectMapper();
    mapper.writeValue(new File(filename), boardList.toArray()); // 첫 번째 파라미터로 파일을 줌! 알아서 하라고~ 간단하게!
  }
